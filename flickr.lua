...
-- Code below is going to be done from inside the Wayback Machine.
...
--  end
  ...
  if allowed(url) and not (string.match(url, "^https?://[^/]*staticflickr%.com/") or string.match(url, "^https?://[^/]*yimg%.com")) then
    html = read_file(file)
    if string.match(html, "hermes%-[0-9]%.[0-9]%.[0-9]+") then
      if string.match(html, "(hermes%-[0-9]%.[0-9]%.[0-9]+)") ~= "hermes-1.1.1114" then
        abortgrab = true
      end
    end
    if string.match(url, "^https://www.flickr.com/photos/[^/]+/[0-9]+/$") and
        --string.match(url, "^https://www.flickr.com/photos/[^/]+/[0-9]+/in/photostream/$")) and
        string.match(html, 'root%.YUI_config%.flickr%.api%.site_key%s+=%s+"[0-9a-f]+"') and
        string.match(html, 'root%.YUI_config%.flickr%.request%.id%s+=%s+"[0-9a-f]+"') then
